...
// 🌟 NEW: Calculate traits dynamically based on the student's category breakdown
const generateFallbackAnalysis = (score: number, total: number, breakdown: any) => {
  ...
  if (percentage >= 80) {
    textData = { overallDNA: "Strategic Architect", focusMore: "Advanced optimization and edge-case system design.", focusLess: "Rote memorization of basic definitions.", actionPlan: "Your core concepts are rock solid. Shift your focus to high-level system design and solving complex, multi-layered algorithmic puzzles under tight time constraints." };
  } else if (percentage >= 50) {
    textData = { overallDNA: "Adaptive Learner", focusMore: "Connecting theoretical concepts to practical applications.", focusLess: "Passively reading without implementing.", actionPlan: "You have a good foundation, but there are gaps in your applied knowledge. Start doing timed mock tests and deeply review the explanations for the questions you miss." };
  } else {
    textData = { overallDNA: "Foundational Builder", focusMore: "Core fundamentals in Data Structures and Operating Systems.", focusLess: "Attempting advanced level mocks prematurely.", actionPlan: "Don't rush to advanced topics. Spend the next 2 weeks purely solidifying your basic concepts. Build mental models and use visual aids to improve retention." };
  }
...
// ------------------------------------------------------------------
// 9. LEARNING DNA: GENERATE DIAGNOSTIC QUIZ (No input needed now!)
// ------------------------------------------------------------------
export async function generateDNAQuizAction() {
  try {
    const apiKeys = [
      process.env.GEMINI_API_KEY1,
      process.env.GEMINI_API_KEY2,
    ...
    if (apiKeys.length > 0) {
      const prompt = `Generate a 5-question baseline Computer Science diagnostic quiz. 
      It MUST contain exactly 1 question for each category: "Concept Understanding", "Problem Solving", "Analytical Ability", and "Memory Retention" plus 1 random.
      Make the "Memory Retention" question a short visual sequence to memorize and set its "type" to "timed_visual". Set the rest to "standard".
      Return ONLY a JSON array with exactly this structure: 
      [{ "category": "...", "type": "...", "question": "...", "options": ["A","B","C","D"], "correctAnswerIndex": 0, "explanation": "..." }]`;
...
// ------------------------------------------------------------------
export async function analyzeDNAResultsAction(breakdown: any, score: number, total: number) {
  try {
    const apiKeys = [
      process.env.GEMINI_API_KEY1,
      process.env.GEMINI_API_KEY2,
    ...
    if (apiKeys.length > 0) {
      const prompt = `A student scored ${score}/${total} on a CS diagnostic test. 
      Here is their category breakdown (correct/total): ${JSON.stringify(breakdown)}. 
      Calculate their cognitive DNA traits on a scale of 1 (Low) to 4 (Ultra) for each category.
      Return ONLY a raw JSON object: 
      { 
        "traits": { "concept": 1-4, "problemSolving": 1-4, "memory": 1-4, "analytical": 1-4 },
        "overallDNA": "Cool Archetype Name (e.g. Memory Savant)", 
    ...
    if (!finalData || !finalData.traits) {
        finalData = generateFallbackAnalysis(score, total, breakdown);
    }
    
    return { success: true, data: finalData };
  } catch (error) {
    return { success: true, data: generateFallbackAnalysis(score, total, breakdown) }; 
  }
}