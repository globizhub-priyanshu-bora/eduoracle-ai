...
import { ArrowLeft, Timer, Trophy, CheckCircle2, XCircle, Loader2, Trash2, Users } from "lucide-react";
  ...
  useEffect(() => {
    if (isFinished) {
      ...
      fetch('/api/save-score', {
        ...
        body: JSON.stringify({ totalScore: finalTotalScore, userId: "test-student-id" })
      })
      .then(res => res.json())
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen bg-neutral-950 text-neutral-50 flex flex-col items-center justify-center p-6">
        ...
        <h2 className="text-xl font-medium animate-pulse text-orange-400">Generating {displaySyllabus} Scenarios...</h2>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-neutral-950 text-neutral-50 flex flex-col p-4 sm:p-6 relative overflow-hidden">
      ...
      <header className="flex items-center justify-between w-full max-w-3xl mx-auto mb-8 z-10">
        <button onClick={() => router.push("/battle")} className="p-2 text-neutral-400 hover:text-white transition-colors bg-white/5 rounded-full border border-white/10">
          <ArrowLeft className="w-5 h-5" />
        </button>
        
        <div className={`flex items-center gap-3 px-4 md:px-6 py-2 rounded-xl border transition-all duration-500 ${
          timeLeft < 60 ? "bg-red-500/20 border-red-500/50 text-red-400 animate-pulse shadow-[0_0_15px_rgba(239,68,68,0.3)]" : "bg-white/5 border-white/10 text-orange-400"
        }`}>
        ...
        <AnimatePresence mode="wait">
          {!isFinished && currentQuestion ? (
            <motion.div key={currentQIndex} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="w-full space-y-6">
              ...
              <div className="grid gap-3">
                {currentQuestion.options.map((option: string, index: number) => {
                  ...
                  if (isAnswered) {
                    if (index === currentQuestion.correctAnswer) {
                      buttonStateClass = "bg-emerald-500/20 border-emerald-500/50 text-emerald-400";
            ...
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="text-center space-y-6 p-8 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-sm w-full">
              ...
              <h2 className="text-3xl font-black text-white">Mission Accomplished!</h2>
              ...
              <div className="bg-black/40 border border-white/10 rounded-2xl p-6 w-full max-w-sm mx-auto space-y-4">
                <div className="flex justify-between items-center text-neutral-300">
                  ...
                  <span className="font-mono font-bold text-2xl">{score} / 100</span>
                </div>
                
                <div className="flex justify-between items-center text-emerald-400">
                  <span className="font-medium tracking-wide">
                    Speed Bonus <span className="text-emerald-500/50 text-xs">({timeLeft}s × 1)</span>
                  </span>
                  <span className="font-mono font-bold text-2xl">+{timeLeft > 0 ? timeLeft : 0}</span>
                </div>
                
                <div className="h-px bg-white/10 w-full my-2" />
                
                <div className="flex justify-between items-end">
                  <span className="font-bold text-neutral-400 uppercase tracking-widest text-sm mb-1">Total XP Rank</span>
              ...
              <button onClick={() => router.push("/")} className="mt-4 px-8 py-4 bg-gradient-to-r from-orange-600 to-red-600 text-white rounded-xl font-bold hover:from-orange-500 hover:to-red-500 transition-all shadow-lg hover:shadow-orange-500/25 active:scale-95 w-full sm:w-auto flex justify-center items-center mx-auto gap-2">
