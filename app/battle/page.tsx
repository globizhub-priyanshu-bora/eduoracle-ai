...
export default function DomainSelectionPage() {
  ...
  const handleProceed = () => {
    if (selectedDomain) {
      // Sends them to the Briefing Page with the domain parameter!
      ...
      >
        <div className="text-center mb-10 space-y-4">
          ...
          <p className="text-neutral-400 max-w-lg mx-auto">
            Choose your engineering specialization or cognitive track to calibrate the AI Battle Engine.
          </p>
        </div>
            ...
            return (
              <motion.button
                ...
                className={`relative flex items-center gap-4 p-5 rounded-2xl border text-left transition-all duration-300 ${
                  isSelected 
                    ? `bg-gradient-to-br ${domain.color} ${domain.borderColor} shadow-[0_0_20px_rgba(0,0,0,0.3)]` 
              ...
              >
                ...
                <div className="flex-1">
                  <h3 className={`font-bold text-base md:text-lg leading-tight ${isSelected ? "text-white" : "text-neutral-200"}`}>
                    {domain.title}
                  </h3>
                ...
                <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                  isSelected ? `border-current ${domain.textColor}` : "border-neutral-600"
                }`}>
                  {isSelected && <motion.div layoutId="select-dot" className="w-2.5 h-2.5 rounded-full bg-current" />}
                </div>
              </motion.button>
            );
          })}
        ...
        <div className="flex justify-center">
          <button 
            ...
            className={`flex items-center gap-2 px-10 py-4 rounded-xl font-bold text-lg transition-all duration-300 ${
              selectedDomain 
                ? "bg-white text-black hover:bg-neutral-200 shadow-[0_0_30px_rgba(255,255,255,0.2)] active:scale-95 cursor-pointer" 
