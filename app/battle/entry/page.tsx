...
import { Calendar, Trophy, Target, Play, ShieldCheck } from "lucide-react";
import { motion } from "framer-motion";

// 1. We renamed this from "export default function BattleQuizEntry" to a standard function
      ...
      >
        <div className="bg-gradient-to-r from-neutral-900 to-neutral-800 p-6 border-b border-white/10">
          ...
          <p className="text-neutral-400 text-xs mt-2 italic">
            Expiry Date: {quizDetails.expiryDate}
          </p>
        </div>
        ...
        <div className="p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 mt-6">
            <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center">
              ...
              <span className="text-[9px] uppercase font-bold text-neutral-500 tracking-tighter">
                Questions
              </span>
            </div>
            <div className="bg-white/5 p-3 rounded-xl border border-white/5 text-center">
...
function StatRow({ icon, label, value, highlight = false }: any) {
  return (
    <div className="flex items-center justify-between text-sm">
      ...
      <span
        className={`font-mono font-bold ${highlight ? "text-orange-400" : "text-neutral-200"}`}
      >
        {value}
      </span>
    </div>
...
// 2. We created a NEW default export that wraps your content in a Suspense boundary
export default function BattleQuizEntry() {
  return (
    <Suspense
      fallback={
        <div className="min-h-screen bg-neutral-950 flex items-center justify-center text-orange-500 font-bold tracking-widest uppercase">
          <div className="animate-pulse">Loading Battle Protocol...</div>
        </div>
      }
    >
      <BattleQuizContent />
