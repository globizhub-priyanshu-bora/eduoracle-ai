      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-white/5 pb-6">
        <div>
          ...
          <p className="text-neutral-400 mt-2">
            ECE-401 Advanced Systems • Semester End Report
          </p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-blue-200/70 mt-1">
              EduGlobiz successfully predicted 9 out of 10 student outcomes
              within a 3% margin of error on day 14 of the semester.
            </p>
          </div>
        </div>
          ...
          return (
            <motion.div
              ...
              className={`relative flex flex-col items-center p-8 rounded-3xl border backdrop-blur-xl ${
                i === 0
                  ? "md:-mt-8 bg-gradient-to-b from-yellow-500/10 to-neutral-900/40 border-yellow-500/20 shadow-[0_0_30px_rgba(234,179,8,0.1)]"
                  : "bg-neutral-900/40 border-white/10"
              }`}
            >
              <div
                className={`absolute -top-6 w-12 h-12 rounded-full border-4 border-neutral-950 flex items-center justify-center ${style.bg} ${style.border}`}
              >
                <RankIcon className={`w-5 h-5 ${style.color}`} />
              </div>

              <div
                className={`w-20 h-20 mt-4 mb-4 rounded-full flex items-center justify-center text-2xl font-black text-white shadow-inner border-2 ${style.border} bg-neutral-800`}
              >
              ...
              <span
                className={`text-[10px] font-bold uppercase tracking-widest mt-1 ${style.color}`}
              >
                Rank {i + 1}
              </span>
              ...
              <div className="mt-6 text-center">
                ...
                <div className="text-xs text-neutral-500 mt-2 font-medium">
                  AI Predicted: {student.predicted}%
                </div>
              </div>
            </motion.div>
          );
            ...
            <tbody className="divide-y divide-white/5 text-sm">
              {remainingStudents.map((student, i) => {
                ...
                return (
                  ...
                  >
                    <td className="p-5 text-center font-mono text-neutral-500 font-bold">
                      {rank}
                    </td>
                    <td className="p-5">
                    ...
                    <td className="p-5 text-center">
                      <span
                        className={`font-black text-lg ${isFailed ? "text-red-400" : "text-white"}`}
                      >
                        {student.score}%
                      </span>
                    </td>
